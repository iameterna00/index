...
contract UnoswapRouter is Pausable, EthReceiver, IUniswapV3SwapCallback {
    ...
    uint256 private constant _SELECTORS = ((uint256(
        uint32(IUniswapV3Pool.token0.selector)
    ) << 224) |
    ...
    function _unoswap(
        ...
        if (protocol == ProtocolLib.Protocol.UniswapV3) {
            ...
            if (spender == address(this)) {
                ...
                IERC20(token.get()).safeTransferFromUniversal(
                    msg.sender,
                    dex.get(),
                    amount,
                    dex.usePermit2()
    ...
    bytes4 private constant _UNISWAP_V2_PAIR_RESERVES_CALL_SELECTOR =
        0x0902f1ac;
    ...
    function _unoswapV2(
        ...
        bytes4 returnAmountNotEnoughException = RouterErrors
        ...
        assembly ("memory-safe") {
            ...
            let zeroForOne := and(
                shr(_UNISWAP_V2_ZERO_FOR_ONE_OFFSET, dex),
                _UNISWAP_V2_ZERO_FOR_ONE_MASK
            )
            let numerator := and(
                shr(_UNISWAP_V2_NUMERATOR_OFFSET, dex),
                _UNISWAP_V2_NUMERATOR_MASK
            )
            if iszero(numerator) {
            ...
            ret := div(
                mul(ret, reserve1),
                add(ret, mul(reserve0, _UNISWAP_V2_DENOMINATOR))
            )
    ...
    uint160 private constant _UNISWAP_V3_MAX_SQRT_RATIO =
    ...
    function _unoswapV3(
        ...
        assembly ("memory-safe") {
            ...
            let pool := and(dex, _ADDRESS_MASK)
            let zeroForOne := and(
                shr(_UNISWAP_V3_ZERO_FOR_ONE_OFFSET, dex),
                _UNISWAP_V3_ZERO_FOR_ONE_MASK
            )
    ...
    function _curfe(
        ...
        assembly ("memory-safe") {
            ...
            function tokenBalanceOf(tokenAddress, accountAddress)
                -> tokenBalance
            {
                mstore(
                    0,
                ...
                if iszero(
                    callReturnSize(
                        staticcall(gas(), tokenAddress, 0, 0x24, 0, 0x20)
                    )
                ) {
                    revert(0, 0)
                }
            ...
            function asmApprove(token, to, value, mem) {
                let
                    selector
                := 0x095ea7b300000000000000000000000000000000000000000000000000000000 // IERC20.approve.selector
                let
                    exception
                := 0x3e3f8f7300000000000000000000000000000000000000000000000000000000 // error ApproveFailed()
            ...
            function curveCoins(pool, selectorOffset, index) -> coin {
                ...
                if iszero(
                    staticcall(gas(), pool, selectorOffset, 0x24, 0, 0x20)
                ) {
                    reRevert()
                }
                coin := mload(0)
            ...
            let hasCallback := and(
                shr(_CURVE_SWAP_HAS_ARG_CALLBACK_OFFSET, dex),
                0x01
            )
            ...
            {
                // Stack too deep
                let toSelectorOffset := and(
                    shr(_CURVE_TO_COINS_SELECTOR_OFFSET, dex),
                    _CURVE_TO_COINS_SELECTOR_MASK
                )
                let toTokenIndex := and(
                    shr(_CURVE_TO_COINS_ARG_OFFSET, dex),
                    _CURVE_TO_COINS_ARG_MASK
                )
            ...
            if and(iszero(hasCallback), or(iszero(useEth), toTokenIsEth)) {
                ...
                let fromTokenIndex := and(
                    shr(_CURVE_FROM_COINS_ARG_OFFSET, dex),
                    _CURVE_FROM_COINS_ARG_MASK
                )
                let fromToken := curveCoins(
                    pool,
                    fromSelectorOffset,
                    fromTokenIndex
                )
            ...
            {
                // stack too deep
                let selectorIndex := and(
                    shr(_CURVE_SWAP_SELECTOR_IDX_OFFSET, dex),
                    _CURVE_SWAP_SELECTOR_IDX_MASK
                )
                if gt(selectorIndex, _CURVE_MAX_SELECTOR_INDEX) {
                    mstore(
                        0,
                        0xa231cb8200000000000000000000000000000000000000000000000000000000
            ...
            mstore(
                add(ptr, 0x04),
                and(shr(_CURVE_FROM_TOKEN_OFFSET, dex), _CURVE_FROM_TOKEN_MASK)
            )
            mstore(
                add(ptr, 0x24),
                and(shr(_CURVE_TO_TOKEN_OFFSET, dex), _CURVE_TO_TOKEN_MASK)
            )
            mstore(add(ptr, 0x44), amount)
            ...
            switch callReturnSize(
                call(
                    gas(),
                    pool,
                    mul(mul(amount, useEth), iszero(toTokenIsEth)),
                    ptr,
                    offset,
                    0,
                    0x40
            ...
            default {
                ret := mload(
                    mul(
                        0x20,
                        and(
                            shr(_CURVE_SWAP_USE_SECOND_OUTPUT_OFFSET, dex),
                            0x01
                        )
                    )
                )
            ...
            if iszero(
                and(shr(_CURVE_SWAP_HAS_ARG_DESTINATION_OFFSET, dex), 0x01)
            ) {
    ...
    function uniswapV3SwapCallback(
        ...
        assembly ("memory-safe") {
            ...
            case 1 {
                if iszero(
                    staticcall(
                        gas(),
                        caller(),
                        add(emptyPtr, _TOKEN0_SELECTOR_OFFSET),
                        0x4,
                        resultPtr,
                        0x20
                    )
            ...
            case 1 {
                ...
                mstore(
                    add(emptyPtr, add(_TRANSFER_SELECTOR_OFFSET, 0x04)),
                    caller()
                )
                mstore(
                    add(emptyPtr, add(_TRANSFER_SELECTOR_OFFSET, 0x24)),
                    amount
                )
                safeERC20(
                    token,
                    0,
                    add(emptyPtr, _TRANSFER_SELECTOR_OFFSET),
                    0x44,
                    0x20
                )
            }
            default {
                ...
                if iszero(
                    staticcall(
                        gas(),
                        caller(),
                        add(emptyPtr, _FEE_SELECTOR_OFFSET),
                        0x4,
                        add(resultPtr, 0x40),
                        0x20
                    )
                ...
                case 1 {
                    ...
                    emptyPtr := add(
                        emptyPtr,
                        _PERMIT2_TRANSFER_FROM_SELECTOR_OFFSET
                    )
    ...
    OrderMixin
{
    ...
    constructor(
        IWETH weth
    ) ClipperRouter(weth) OrderMixin(weth) Ownable(msg.sender) {
